DROP FUNCTION IF EXISTS test_griddify(rast raster, init boolean);
CREATE OR REPLACE FUNCTION test_griddify_s(rast raster, init boolean) 
    RETURNS boolean AS $$
    DECLARE 
    ...
    BEGIN
        IF init THEN
            DROP TABLE IF EXISTS griddify_results;
            CREATE TEMPORARY TABLE griddify_results (
                    the_geom geometry,
                    upper_left geometry,
                    upper_left_x numeric,
                    upper_left_y numeric,
                    sparse_time_series int[],
                    time_series int[],
                    cummulative int[],
                    total_incr int,
                    pixels int,
        ...
        FOR a IN 1..(rastw-1) BY s LOOP 
            FOR b IN 1..(rasth-1) BY s LOOP 
                ...
                upper_left_x := floor(ST_XMin(poly));
                upper_left_y := floor(ST_YMax(poly));
                ...
                IF 0 < total_incr THEN
                    ...
                    FOR i in array_lower(adder, 1) .. array_upper(adder, 1) LOOP
                        ...
                        ct := holder[i] + ct;
                    END LOOP;
                    INSERT INTO griddify_results_a (boxpoly, upper_left, upper_left_x, upper_left_y, sparse_time_series, time_series, cummulative, total_incr, pixels, cell_width, cell_height) VALUES (poly, upper_left, upper_left_x, upper_left_y, sparsets, holder, adder, total_incr, (s*s), (s*ST_PixelWidth(rast))::int, (2*ST_PixelHeight(rast))::int);
                END IF;
            END LOOP;
        END LOOP;
        RETURN TRUE;
...
SELECT test_griddify(rast, false) FROM amc WHERE rid = 3562;
UPDATE griddify_results SET the_geom = ST_Multi(ST_Transform(boxpoly,4326));
SELECT * FROM griddify_results ORDER BY total_incr DESC;
